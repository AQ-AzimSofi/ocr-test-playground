...
export interface TextMatch {
  gemini_index: number;
  cloud_vision_index: number;
  gemini_text: string;
  cloud_vision_text: string;
  match_confidence: number;
  match_reason: string;
}

export interface UnmatchedGeminiText {
  gemini_index: number;
  text: string;
...
export interface TextMatchingOutput {
  matched_pairs: TextMatch[];
  unmatched_gemini_texts: UnmatchedGeminiText[];
  matching_stats: {
    total_gemini_texts: number;
    total_cloud_vision_texts: number;
    matched_count: number;
    unmatched_gemini_count: number;
...
Return a JSON object with the complete matching results.`;
  ...
  try {
    ...
    return parsedOutput as TextMatchingOutput;
  } catch (error) {
...
Return a valid JSON object with the "verified_texts" array containing all results.`;
  ...
  try {
    const parsedOutput = JSON.parse(jsonText);
    return parsedOutput.verified_texts as VerifiedText[];
  } catch (error) {
    console.error('Failed to parse verification output:', error);
    console.error('Raw response:', jsonText);
    throw new Error('Invalid output from Text Verification Agent');
  }
}
