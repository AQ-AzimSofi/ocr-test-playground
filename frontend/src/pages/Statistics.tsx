...
export function Statistics() {
  ...
  const confidenceDistribution = tools.map((tool) => {
    ...
    const good = bboxes.filter((b) => (b.confidence || 1) >= 0.85 && (b.confidence || 1) < 0.95).length;
    const medium = bboxes.filter((b) => (b.confidence || 1) >= 0.75 && (b.confidence || 1) < 0.85).length;
  ...
  const geminiUpdateStats = tools.map((tool) => {
    const bboxes = resultsByTool[tool]?.boundingBoxes || [];
    const geminiUpdates = bboxes.filter((b) => b.metadata?.geminiUpdated).length;
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="text-3xl font-bold text-gray-900 mt-2">{tools.length}</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-sm text-gray-600">Avg Processing Time</div>
            <div className="text-3xl font-bold text-blue-600 mt-2">
              {Math.round(
                processingTimeData.reduce((sum, d) => sum + d.time, 0) / tools.length
              )}
              ms
            </div>
          </div>
        ...
        {/* Processing Time */}
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">Processing Time by Tool</h2>
          <ResponsiveContainer width="100%" height={300}>
        ...
        {/* Bounding Box Count */}
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">Bounding Box Count by Tool</h2>
          <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">Confidence Distribution by Tool</h2>
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={confidenceDistribution}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="excellent" stackId="a" fill="#22c55e" name="Excellent (≥95%)" />
              <Bar dataKey="good" stackId="a" fill="#eab308" name="Good (85-95%)" />
              <Bar dataKey="medium" stackId="a" fill="#f97316" name="Medium (75-85%)" />
        ...
        {/* Gemini Update Statistics */}
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">Gemini Update Statistics</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={geminiUpdateStats}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="geminiUpdates" fill="#8b5cf6" name="Gemini Updates" />
                <Bar dataKey="lowConfidence" fill="#ef4444" name="Low Confidence Regions" />
              </BarChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={geminiUpdateStats}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="tool" />
                <YAxis label={{ value: 'Update Rate (%)', angle: -90, position: 'insideLeft' }} />
                <Tooltip />
                <Legend />
                <Line
                  type="monotone"
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right font-semibold">Processing Time</th>
                  <th className="px-4 py-3 text-right font-semibold">API Cost</th>
                  <th className="px-4 py-3 text-right font-semibold">BBoxes</th>
                  <th className="px-4 py-3 text-right font-semibold">Low Confidence</th>
                  <th className="px-4 py-3 text-right font-semibold">Gemini Updates</th>
                  <th className="px-4 py-3 text-right font-semibold">Update Rate</th>
                  {accuracyData.length > 0 && (
                    <>
                      <th className="px-4 py-3 text-right font-semibold">CER</th>
                      <th className="px-4 py-3 text-right font-semibold">WER</th>
                    </>
                  )}
                </tr>
              </thead>
              <tbody className="divide-y">
                {tools.map((tool, idx) => {
                  ...
                  return (
                    <tr key={`stats-${tool}-${idx}`} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium">{tool}</td>
                      <td className="px-4 py-3 text-right">{result?.processingTimeMs || 0}ms</td>
                      <td className="px-4 py-3 text-right">¥{(result?.apiCost || 0).toFixed(2)}</td>
                      <td className="px-4 py-3 text-right">{stats.totalBboxes}</td>
                      <td className="px-4 py-3 text-right">{stats.lowConfidence}</td>
                      <td className="px-4 py-3 text-right">{stats.geminiUpdates}</td>
                      <td className="px-4 py-3 text-right">{stats.updateRate.toFixed(1)}%</td>
                      {accuracyData.length > 0 && (
                        <>
                          <td className="px-4 py-3 text-right">
                            {accuracy?.cer ? `${accuracy.cer.toFixed(2)}%` : 'N/A'}
                          </td>
                          <td className="px-4 py-3 text-right">
                            {accuracy?.wer ? `${accuracy.wer.toFixed(2)}%` : 'N/A'}
                          </td>
                        </>
                      )}
                    </tr>
