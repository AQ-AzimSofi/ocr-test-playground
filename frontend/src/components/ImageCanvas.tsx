...
export const ImageCanvas = forwardRef<ImageCanvasRef, ImageCanvasProps>(function ImageCanvas({
  ...
  useEffect(() => {
    ...
    const minTranslateX = Math.min(0, canvasWidth - canvasWidth * targetScale);
    const minTranslateY = Math.min(0, canvasHeight - canvasHeight * targetScale);

    const clampedTranslateX = Math.max(minTranslateX, Math.min(0, newTranslateX));
    const clampedTranslateY = Math.max(minTranslateY, Math.min(0, newTranslateY));
  ...
  useEffect(() => {
    ...
    filteredBBoxes.forEach((bbox, index) => {
      ...
      if (normalizedBounds.length < 3) {
        console.warn('[ImageCanvas] Skipping bbox with insufficient points after normalization:', {
          index,
          text: bbox.text,
  ...
  }, [image, filteredBBoxes, hoveredIndex, selectedIndex, heatmapMode, showGeminiIcons, showFill, scale, translateX, translateY]);
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent<HTMLCanvasElement>) => {
      ...
      const minTranslateX = Math.min(0, canvas.width - canvas.width * newScale);
      const minTranslateY = Math.min(0, canvas.height - canvas.height * newScale);
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-auto bg-gray-900 rounded-lg">
      <canvas
        ref={canvasRef}
