 ...
 */
export async function processWithGeminiValidation(imagePath: string, drawingId: string) {
  ...
  try {
    ...
    console.log(`  Azure extracted: ${azureResult.content.length} chars, ${azureResult.words.length} words`);
...
- If everything is correct, say "No issues found"`;
    ...
    const validationResponse = await geminiClient.extractWithCustomPrompt(imagePath, validationPrompt);

    console.log(`  Gemini validation response preview: ${validationResponse.substring(0, 300)}...`);

    // Step 3: Parse validation results
    const validation = parseGeminiValidation(validationResponse);

    console.log(`  Gemini found: ${validation.missingText.length} missing, ${validation.incorrectText.length} incorrect`);

    // If no issues found, return Azure results as-is
    if (validation.missingText.length === 0 && validation.incorrectText.length === 0) {
      ...
      console.log(`  Gemini Validation completed in ${(processingTime / 1000).toFixed(2)}s`);
    ...
    for (const missing of validation.missingText) {
      ...
      if (estimatedPosition) {
        console.log(`  Estimated position from description: (${estimatedPosition.x.toFixed(0)}, ${estimatedPosition.y.toFixed(0)})`);

        // Crop region around estimated position
        const cropSize = 200; // Pixels
        const cropRegion = {
          bounds: [
            { x: Math.max(0, estimatedPosition.x - cropSize / 2), y: Math.max(0, estimatedPosition.y - cropSize / 2) },
            { x: Math.min(imageWidth, estimatedPosition.x + cropSize / 2), y: Math.max(0, estimatedPosition.y - cropSize / 2) },
            { x: Math.min(imageWidth, estimatedPosition.x + cropSize / 2), y: Math.min(imageHeight, estimatedPosition.y + cropSize / 2) },
            { x: Math.max(0, estimatedPosition.x - cropSize / 2), y: Math.min(imageHeight, estimatedPosition.y + cropSize / 2) },
          ],
          text: '',
          confidence: 0,
        ...
        try {
          ...
          const geminiResult = await geminiClient.extractTextFromRegion(cropped.base64);
          ...
          console.log(`  Extracted missing text from region: "${geminiResult.text.substring(0, 50)}..."`);
        } catch (error) {
    ...
    for (const incorrect of validation.incorrectText) {
      console.log(`  Correcting: "${incorrect.found}" -> "${incorrect.shouldBe}"`);
      ...
      if (match) {
        ...
        console.warn(`  Could not find bbox for incorrect text: "${incorrect.found}"`);
      }
    }
    ...
    const finalText = azureResult.content + '\n' + synthesizedBboxes.map(b => b.text).join('\n');

    const processingTime = Date.now() - startTime;
    const azureCost = azureDocumentClient.estimateCost(azureResult.pages.length, 'read');
    const geminiCost = geminiClient.estimateCost(1 + additionalGeminiCalls, false);
    const totalCost = azureCost + geminiCost;
    ...
    const [dbResult] = await db
      ...
      .values({
        ...
        metadata: {
          azureWordCount: azureResult.words.length,
          geminiValidationIssues: validation.missingText.length + validation.incorrectText.length,
    ...
    console.log(`     Total bboxes: ${allBboxes.length} (${azureResult.words.length} Azure + ${synthesizedBboxes.length} synthesized)`);
