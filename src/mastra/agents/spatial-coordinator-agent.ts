...
const BatchCalibratedCoordinateSchema = z.object({
  text: z.string(),
...
const BatchOutputSchema = z.object({
  coordinates: z.array(BatchCalibratedCoordinateSchema),
});

export type BatchCalibratedCoordinate = z.infer<
  typeof BatchCalibratedCoordinateSchema
>;
...
Return a valid JSON object with the "coordinates" array containing all results.`;
  ...
  try {
    ...
    const validatedOutput = BatchOutputSchema.parse(parsedOutput);

    const processedCoordinates = validatedOutput.coordinates.map((coord) => {
...
function fallbackCalibration(
  ...
  return {
    ...
    confidence: calibrationOffset.confidence * 0.7,
